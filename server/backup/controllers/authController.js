...
class AuthController {
    constructor() {
        ...
        this.notificationService = new notificationService_1.NotificationService({ daysBeforeExpiration: [] });
    }
    async register(req, res) {
        try {
            ...
            // Créer le token de vérification si email fourni
            let verificationToken;
            if (email) {
                verificationToken = jsonwebtoken_1.default.sign({ email }, jwt_config_1.default.verificationSecret, { expiresIn: '24h' });
            }
            // Créer l'utilisateur
            const user = await User_1.User.create({
                ...
                verificationToken,
                role: 'user',
                isVerified: !email // Si pas d'email, pas besoin de vérification
            });
