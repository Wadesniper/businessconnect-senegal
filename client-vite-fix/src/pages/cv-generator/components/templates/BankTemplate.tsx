...
const BankTemplate: React.FC<BankTemplateProps> = ({ data }) => {
  return (
    <div style={{ background: 'linear-gradient(135deg, #f0f5ff 0%, #fffbe6 100%)', borderRadius: 16, boxShadow: '0 4px 24px rgba(24, 24, 24, 0.08)', padding: 32 }}>
      {/* En-tête */}
      <Row gutter={24} align="middle" style={{ marginBottom: 32 }}>
        <Col xs={24} md={7} style={{ textAlign: 'center' }}>
          ...
          <Text style={{ color: '#faad14' }}>{data.personalInfo.title}</Text>
        </Col>
        <Col xs={24} md={17}>
          ...
          <Paragraph style={{ marginTop: 24, fontSize: 16, color: '#1d39c4' }}>{data.summary}</Paragraph>
        </Col>
      </Row>

      {/* Expérience */}
      <section style={{ marginBottom: 32 }}>
        <Title level={4} style={{ color: '#faad14' }}><DollarOutlined /> Expérience Banque / Assurance</Title>
        <Timeline>
