...
const AgroTemplate: React.FC<AgroTemplateProps> = ({ data, isMiniature = false, customization }) => {
  const primaryColor = customization?.primaryColor || '#237804';
  const secondaryColor = '#52c41a'; // Couleur secondaire pour les accents
  ...
  return (
    <div style={{ width: 794, background: blanc, borderRadius: 18, overflow: 'hidden', fontFamily: 'Montserrat, Arial, sans-serif', boxShadow: '0 4px 24px #0002', display: 'flex', flexDirection: 'column' }}>
      {/* Header bandeau avec couleur personnalisée */}
      <div style={{ width: '100%', background: primaryColor, minHeight: 140, display: 'flex', flexDirection: 'row', alignItems: 'center', padding: 0, position: 'relative', zIndex: 2 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', flex: 1, background: blanc, minHeight: 600 }}>
        {/* Colonne gauche */}
        <div style={{ width: '32%', background: blanc, color: primaryColor, display: 'flex', flexDirection: 'column', padding: '32px 0 0 32px', gap: 32 }}>
          {/* Informations */}
          {summary && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <span style={{ fontWeight: 700, fontSize: 15, color: primaryColor, letterSpacing: 1, textTransform: 'uppercase' }}>Informations</span>
                <span style={{ flex: 1, height: 2, background: secondaryColor, borderRadius: 1, marginLeft: 8, minWidth: 40 }} />
              </div>
          ...
          {skills.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <span style={{ fontWeight: 700, fontSize: 15, color: primaryColor, letterSpacing: 1, textTransform: 'uppercase' }}>Compétences</span>
                <span style={{ flex: 1, height: 2, background: secondaryColor, borderRadius: 1, marginLeft: 8, minWidth: 40 }} />
              </div>
          ...
          {interests.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <span style={{ fontWeight: 700, fontSize: 15, color: primaryColor, letterSpacing: 1, textTransform: 'uppercase' }}>Intérêts</span>
                <span style={{ flex: 1, height: 2, background: secondaryColor, borderRadius: 1, marginLeft: 8, minWidth: 40 }} />
              </div>
              <ul style={{ paddingLeft: 0, margin: 0, listStyle: 'none' }}>
          ...
          {languages.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <span style={{ fontWeight: 700, fontSize: 15, color: primaryColor, letterSpacing: 1, textTransform: 'uppercase' }}>Langues</span>
                <span style={{ flex: 1, height: 2, background: secondaryColor, borderRadius: 1, marginLeft: 8, minWidth: 40 }} />
              </div>
              <ul style={{ paddingLeft: 0, margin: 0, listStyle: 'none' }}>
          ...
          {education.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <span style={{ fontWeight: 700, fontSize: 15, color: primaryColor, letterSpacing: 1, textTransform: 'uppercase' }}>Formations</span>
                <span style={{ flex: 1, height: 2, background: secondaryColor, borderRadius: 1, marginLeft: 8, minWidth: 40 }} />
              </div>
              {education.map((edu, idx) => (
                <div key={idx} style={{ background: blanc, borderRadius: 12, boxShadow: '0 2px 8px #0001', padding: '14px 18px', marginBottom: 14, border: `1px solid ${gris}` }}>
                  ...
                  <div style={{ fontSize: 12, color: secondaryColor }}>{edu.startDate} - {edu.endDate}</div>
        ...
        {/* Colonne droite */}
        <div style={{ width: '68%', background: blanc, color: primaryColor, display: 'flex', flexDirection: 'column', padding: '32px 48px 32px 48px', gap: 32 }}>
          {/* Expériences professionnelles */}
          {experience.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 18 }}>
                <span style={{ fontWeight: 700, fontSize: 16, color: primaryColor, textTransform: 'uppercase', letterSpacing: 1 }}>Expériences professionnelles</span>
                <span style={{ flex: 1, height: 2, background: secondaryColor, borderRadius: 1, marginLeft: 8, minWidth: 40 }} />
              </div>
              {experience.map((exp, idx) => (
                <div key={idx} style={{ background: blanc, borderRadius: 12, boxShadow: '0 2px 8px #0001', padding: '18px 24px', marginBottom: 18, border: `1px solid ${gris}` }}>
                  ...
                  <div style={{ fontSize: 12, color: secondaryColor }}>{exp.startDate} - {exp.current ? 'Présent' : exp.endDate}</div>
