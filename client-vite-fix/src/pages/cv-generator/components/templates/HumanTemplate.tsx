...
import { MailOutlined, PhoneOutlined, EnvironmentOutlined, LinkedinOutlined, GlobalOutlined, HeartOutlined } from '@ant-design/icons';
...
const HumanTemplate: React.FC<HumanTemplateProps> = ({ data, isMiniature = false }) => {
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, #e6f7ff 0%, #fff0f6 100%)', borderRadius: isMiniature ? 8 : 16, boxShadow: '0 4px 24px rgba(255, 77, 153, 0.08)', padding: isMiniature ? 12 : 32, fontSize: isMiniature ? 10 : 16 }}>
      {/* En-tête */}
      <Row gutter={isMiniature ? 8 : 24} align="middle" style={{ marginBottom: isMiniature ? 12 : 32 }}>
        <Col xs={24} md={7} style={{ textAlign: 'center' }}>
          <Avatar
            src={data.personalInfo.photo || '/images/avatars/woman-2.png'}
      ...
      <section style={{ marginBottom: isMiniature ? 8 : 32 }}>
        <Title level={isMiniature ? 5 : 4} style={{ color: '#2f54eb', fontSize: isMiniature ? 11 : 18 }}><HeartOutlined /> Expérience Humanitaire / Social</Title>
        <Timeline>
        ...
        <Row gutter={[isMiniature ? 4 : 16, isMiniature ? 4 : 16]}>
          {skills.map((skill, idx) => (
            <Col xs={24} sm={12} md={8} key={idx} style={{ marginBottom: isMiniature ? 2 : 8 }}>
              <Card style={{ background: '#e6f7ff', border: '1px solid #adc6ff', padding: isMiniature ? 4 : 12, fontSize: isMiniature ? 9 : 13 }}>
      ...
      {Array.isArray(data.references) && data.references.length > 0 && (
        <div style={{ marginTop: 24 }}>
          <div style={{ fontWeight: 700, fontSize: 18, color: '#ad1457', marginBottom: 8 }}>RÉFÉRENCES</div>
          {data.references.map((ref: any, idx: number) => (
            <div key={idx} style={{ fontSize: 14, color: '#ad1457', marginBottom: 8 }}>
