...
const cardShadowStyle = '8px 8px 0 #4b6c5c, 0 6px 24px #0003';
...
const CardWindow: React.FC<{ title: string; children: React.ReactNode; style?: React.CSSProperties }> = ({ title, children, style }) => (
  <div style={{ background: blanc, border: borderStyle, borderRadius: 12, boxShadow: cardShadowStyle, marginBottom: 24, position: 'relative', ...style }}>
    ...
    <div style={{ position: 'absolute', right: 14, top: 10, display: 'flex', gap: 6, zIndex: 2 }}>
      <span style={{ width: 16, height: 16, borderRadius: '50%', background: '#eaf3ee', border: '2px solid #4b6c5c', display: 'inline-block', textAlign: 'center', fontWeight: 700, fontSize: 12, color: '#4b6c5c', lineHeight: '12px', padding: 0 }}>-</span>
    ...
    <div style={{ width: 794, minHeight: 1123, background: blanc, fontFamily: 'Montserrat, Arial, sans-serif', borderRadius: 18, boxShadow: shadowStyle, padding: 0, border: borderStyle }}>
      ...
      <CardWindow title="Profil" style={{ margin: '0 24px 24px 24px' }}>
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', gap: 24 }}>
          {/* Photo de profil */}
          {personalInfo.photo && (
            <img src={personalInfo.photo} alt="avatar" style={{ width: 100, height: 100, borderRadius: 14, objectFit: 'cover', boxShadow: '0 2px 12px #0004', border: `3px solid ${vertFonce}`, marginRight: 18 }} />
          )}
          {/* Infos à droite */}
