...
const CommerceTemplate: React.FC<CommerceTemplateProps> = ({ data }) => {
  return (
    <div style={{ background: 'linear-gradient(135deg, #fff1f0 0%, #ffd6e7 100%)', borderRadius: 16, boxShadow: '0 4px 24px rgba(255, 77, 79, 0.08)', padding: 32 }}>
      {/* En-tête */}
      <Row gutter={24} align="middle" style={{ marginBottom: 32 }}>
        <Col xs={24} md={7} style={{ textAlign: 'center' }}>
          <Avatar
            src={data.personalInfo.photo || '/images/avatars/man-3.png'}
            size={160}
            style={{ border: '4px solid #ff4d4f', marginBottom: 16 }}
          />
          <Title level={3} style={{ color: '#cf1322', margin: 0 }}>
            {data.personalInfo.firstName} {data.personalInfo.lastName}
          </Title>
          <Text style={{ color: '#a8071a' }}>{data.personalInfo.title}</Text>
        </Col>
        <Col xs={24} md={17}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
            <Tag color="red"><MailOutlined /> {data.personalInfo.email}</Tag>
            <Tag color="volcano"><PhoneOutlined /> {data.personalInfo.phone}</Tag>
            <Tag color="magenta"><EnvironmentOutlined /> {data.personalInfo.address}</Tag>
          ...
          <Paragraph style={{ marginTop: 24, fontSize: 16, color: '#a8071a' }}>{data.summary}</Paragraph>
        </Col>
      </Row>

      {/* Expérience */}
      <section style={{ marginBottom: 32 }}>
        <Title level={4} style={{ color: '#ff4d4f' }}><ShoppingCartOutlined /> Expérience Commerce</Title>
        <Timeline>
          {data.experience.map((exp, idx) => (
            <Timeline.Item key={idx} color="#ff4d4f">
              <Card style={{ borderLeft: '4px solid #ff4d4f', marginBottom: 16 }}>
                <Title level={5} style={{ color: '#cf1322' }}>{exp.title}</Title>
      ...
      {/* Compétences */}
      <section style={{ marginBottom: 32 }}>
        <Title level={4} style={{ color: '#ff4d4f' }}>Compétences Clés</Title>
        <Row gutter={[16, 16]}>
      ...
      {/* Formation */}
      <section style={{ marginBottom: 32 }}>
        <Title level={4} style={{ color: '#ff4d4f' }}>Formation</Title>
        <Timeline>
      ...
      {data.certifications && data.certifications.length > 0 && (
        <section style={{ marginBottom: 32 }}>
          <Title level={4} style={{ color: '#ff4d4f' }}>Certifications</Title>
          <Row gutter={[16, 16]}>
      ...
      {/* Langues */}
      <section>
        <Title level={4} style={{ color: '#ff4d4f' }}>Langues</Title>
        <Row gutter={[16, 16]}>
          {data.languages.map((lang, idx) => (
            <Col key={idx} xs={12} sm={8} md={6}>
              <Tag color="red" style={{ fontSize: 16 }}>{lang.name} - {lang.level}</Tag>
            </Col>
          ))}
        </Row>
      </section>
