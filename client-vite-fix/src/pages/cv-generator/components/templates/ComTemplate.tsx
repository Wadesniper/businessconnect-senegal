...
const ComTemplate: React.FC<ComTemplateProps> = ({ data, isMiniature = false, customization }) => {
  ...
  return (
    <div className="cv-template-container" style={{ width: 794, minHeight: 1123, background: bleuClair, borderRadius: 18, overflow: 'hidden', fontFamily: 'Montserrat, Arial, sans-serif', display: 'flex', flexDirection: 'row', boxShadow: '0 4px 24px #0002', position: 'relative' }}>
      ...
      <div style={{ width: '34%', background: blanc, color: primaryColor, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 0 32px 0', gap: 32 }}>
        {/* Photo + nom + titre */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 32 }}>
          ...
          <div style={{ fontWeight: 900, fontSize: 28, color: primaryColor, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 2, wordBreak: 'break-word', overflowWrap: 'break-word' }}>{personalInfo.firstName} {personalInfo.lastName}</div>
          <div style={{ fontSize: 16, color: primaryColor, fontWeight: 500, marginBottom: 10, textTransform: 'capitalize', wordBreak: 'break-word', overflowWrap: 'break-word' }}>{personalInfo.title}</div>
        </div>
        {/* Contact */}
        <div style={{ width: '80%', marginBottom: 24 }}>
          <div style={{ fontWeight: 700, fontSize: 15, color: primaryColor, marginBottom: 8, letterSpacing: 1, textTransform: 'uppercase', background: bleuClair, padding: '4px 16px', borderRadius: 8, display: 'inline-block' }}>Contact</div>
        ...
        {skills.length > 0 && (
          <div style={{ width: '80%', marginBottom: 24 }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: primaryColor, marginBottom: 8, letterSpacing: 1, textTransform: 'uppercase', background: bleuClair, padding: '4px 16px', borderRadius: 8, display: 'inline-block' }}>Compétences</div>
            <div style={{ height: 2, width: 32, background: primaryColor, opacity: 0.12, marginBottom: 12, borderRadius: 2 }} />
        ...
        {education.length > 0 && (
          <div style={{ width: '80%', marginBottom: 24 }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: primaryColor, marginBottom: 8, letterSpacing: 1, textTransform: 'uppercase', background: bleuClair, padding: '4px 16px', borderRadius: 8, display: 'inline-block' }}>Formation</div>
            <div style={{ height: 2, width: 32, background: primaryColor, opacity: 0.12, marginBottom: 12, borderRadius: 2 }} />
      ...
      <div style={{ width: '66%', background: bleuClair, color: primaryColor, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'flex-start', padding: '48px 40px 32px 40px', gap: 32 }}>
        {/* Résumé */}
        {summary && <div style={{ fontSize: 15, color: primaryColor, marginBottom: 18 }}>{summary}</div>}
        {/* Expérience */}
        {experience.length > 0 && (
          <div style={{ width: '100%', marginBottom: 24 }}>
            <div style={{ fontWeight: 700, fontSize: 16, color: primaryColor, marginBottom: 14, textTransform: 'uppercase', letterSpacing: 1, background: bleuClair, padding: '4px 16px', borderRadius: 8, display: 'inline-block' }}>Expérience</div>
        ...
        {languages.length > 0 && (
          <div style={{ width: '100%', marginBottom: 24 }}>
            <div style={{ fontWeight: 700, fontSize: 16, color: primaryColor, marginBottom: 14, textTransform: 'uppercase', letterSpacing: 1, background: bleuClair, padding: '4px 16px', borderRadius: 8, display: 'inline-block' }}>Langues</div>
            <ul style={{ paddingLeft: 18, margin: 0 }}>
              {languages.map((lang, i) => <li key={i} style={{ fontSize: 13, color: primaryColor, marginBottom: 4 }}>{lang.name} - {lang.level}</li>)}
            </ul>
          </div>
        )}
      </div>
