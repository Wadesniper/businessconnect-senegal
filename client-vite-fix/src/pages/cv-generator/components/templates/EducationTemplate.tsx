...
const EducationTemplate: React.FC<EducationTemplateProps> = ({ data }) => {
  return (
    <div style={{ background: 'linear-gradient(135deg, #f0f5ff 0%, #d6e4ff 100%)', borderRadius: 16, boxShadow: '0 4px 24px rgba(24, 144, 255, 0.08)', padding: 32 }}>
      {/* En-tête */}
      <Row gutter={24} align="middle" style={{ marginBottom: 32 }}>
        <Col xs={24} md={7} style={{ textAlign: 'center' }}>
          <Avatar
            ...
            size={160}
            style={{ border: '4px solid #2f54eb', marginBottom: 16 }}
          />
        ...
        <Col xs={24} md={17}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
            ...
            <Tag color="purple"><EnvironmentOutlined /> {data.personalInfo.address}</Tag>
      ...
      <section style={{ marginBottom: 32 }}>
        <Title level={4} style={{ color: '#2f54eb' }}><BookOutlined /> Expérience Éducation</Title>
        <Timeline>
          {data.experience.map((exp, idx) => (
            <Timeline.Item key={idx} color="#2f54eb">
              <Card style={{ borderLeft: '4px solid #2f54eb', marginBottom: 16 }}>
